...
# Change Log
#     example:NtripClientwithserial.py -u evanbandit@gmail.com -p none -m 100000 -r 100 -v -k /dev/ttyUSB0 -2 3.23.52.207 2101 BeehiveBeach
#     support functions:   
#         1.get source table;
...
#                        print('recv:', casterResponse.decode(encoding='utf-8'))    # print recv data(for debug)
                        ...
                        for line in header_lines:
                            if line.find("SOURCETABLE") >= 0:
                                ...
                                self.socket.sendall(self.getGGAString().encode(encoding='utf-8'))
                                uploadTime = datetime.datetime.now()  # get current upload time
#below here is where the data is sent, need to change that to usb0 serial 
...
if __name__ == '__main__':
    ...
    parser.add_option("-k", "--sserial", type = "string", default = None, help = "Broadcast recieved data on the provided serial port")
    ...
    if options.sserial:
        ntripArgs['serial_port'] = options.sserial
        usb = serial.Serial(options.sserial, baudrate=38400, timeout=1)
        print("Began serial connection")
